...
Usage:
    ...
    python3 scripts/qy_scheduler.py start

    # Run a single scheduled task
    python3 scripts/qy_scheduler.py run-daily
    python3 scripts/qy_scheduler.py run-weekly
    python3 scripts/qy_scheduler.py run-monthly

    # Show schedule status
    python3 scripts/qy_scheduler.py status
"""
...
def run_daily() -> Dict[str, Any]:
    ...
    # Invoke qy_alerts.py as a subprocess
    alerts_script = _SCRIPTS_DIR / "qy_alerts.py"
...
def run_weekly() -> Dict[str, Any]:
    ...
    For each ticker:
      ...
      2. Recalculate GG via calculate_qy_gg.py
    ...
    gg_script = _SCRIPTS_DIR / "calculate_qy_gg.py"
...
def _write_weekly_report(
    ...
    lines.append("---")
    lines.append("")
    lines.append("*Generated by Quality Yield Strategy Weekly Scheduler*")
...
def _write_monthly_report(
    ...
    if not_in_screen:
        ...
        for ticker in sorted(not_in_screen):
            lines.append(
                ...
                f"Review if still meets Quality Yield criteria."
            )
        lines.append("")
    ...
    lines.append("---")
    lines.append("")
    lines.append("*Generated by Quality Yield Strategy Monthly Scheduler*")
...
def start_daemon(config: Dict[str, Any]) -> None:
    ...
    print(f"\n{'='*70}")
    print("QUALITY YIELD STRATEGY SCHEDULER -- Daemon Mode")
    print(f"{'='*70}")
    print(f"  Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
...
def show_status() -> None:
    ...
    print(f"\n{'='*70}")
    print("QUALITY YIELD STRATEGY SCHEDULER -- Status")
    print(f"{'='*70}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Quality Yield Strategy Scheduler -- US Equity",
