#!/usr/bin/env python3
"""Daily Alert Monitor for US Equity Turtle Strategy.
...
Usage:
    python3 scripts/turtle_alerts.py                        # Check all holdings
    python3 scripts/turtle_alerts.py --tickers AAPL MSFT    # Check specific tickers
"""
    ...
    if price <= week52_low:
        alerts.append(Alert(
            ...
            message=(
                ...
                f"Potential buying opportunity per Turtle Strategy."
            ),
            details={
                "price": price,
    ...
    if price >= week52_high:
        alerts.append(Alert(
            ...
            message=(
                ...
                f"Consider trimming per Turtle Strategy."
            ),
            details={
                "price": price,
...
def format_alert_report(
    ...
    lines = []
    lines.append(f"# Turtle Strategy Daily Alerts -- {today}")
    lines.append("")
    ...
    lines.append("---")
    lines.append("")
    lines.append("*Generated by Turtle Strategy Daily Alert Monitor*")
...
def run_alerts(tickers: List[str]) -> Dict[str, Any]:
    ...
    print(f"\n{'='*70}")
    print("TURTLE STRATEGY -- Daily Alert Monitor")
    print(f"{'='*70}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Turtle Strategy Daily Alert Monitor -- US Equity",
    ...
    if args.tickers:
        ...
        if not tickers:
            print(
                ...
                "  2. Specify tickers directly: python3 scripts/turtle_alerts.py --tickers AAPL MSFT",
                file=sys.stderr,
            )
            sys.exit(1)
