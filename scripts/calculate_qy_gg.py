#!/usr/bin/env python3
"""
US Equity Quality Yield GG Calculator
...
Single-ticker usage:
    python3 scripts/calculate_qy_gg.py \\
...
# ---------------------------------------------------------------------------
# Allow running from project root  (python3 scripts/calculate_qy_gg.py)
# or from the scripts/ directory   (python3 calculate_qy_gg.py).
...
# Logging
# ---------------------------------------------------------------------------
logger = logging.getLogger("qy_gg")
...
def _columns_are_descending(rows: List[Dict[str, Any]]) -> bool:
    """Detect if table columns are in descending year order (newest first).

    Bloomberg data packs use ascending order (2021 → 2025); yfinance uses
    descending order (2025 → 2021).  We parse year-like column headers to
    ...
    for key in first_row:
        # Extract 4-digit year from header like "2025", "2025-11-28", etc.
        m = re.match(r"(\d{4})", str(key))
        if m:
            years.append(int(m.group(1)))
    if len(years) >= 2:
        return years[0] > years[-1]  # first col year > last col year → descending
    return False
...
def _latest_value(rows: List[Dict[str, Any]], metric_key: str) -> float:
    ...
    label via ``_METRIC_LABEL_MAP`` and then return the most-recent
    numeric cell in that row.  Column ordering (ascending or descending
    by year) is auto-detected.
    """
    if not rows:
        return 0.0
    ...
    for row in rows:
        ...
        if canonical == metric_key:
            ...
            if values:
                # descending → most recent is first; ascending → most recent is last
                return float(values[0]) if descending else float(values[-1])
    return 0.0
...
def _latest_two_values(rows: List[Dict[str, Any]], metric_key: str) -> Tuple[float, float]:
    ...
    Column ordering (ascending or descending by year) is auto-detected.
    """
    if not rows:
        return 0.0, 0.0
    ...
    for row in rows:
        ...
        if canonical == metric_key:
            ...
            if len(values) >= 2:
                if descending:
                    # values[0] = latest, values[1] = previous
                    return float(values[1]), float(values[0])
                else:
                    # values[-1] = latest, values[-2] = previous
...
def calculate_gg(
    ...
    """Calculate Quality Yield GG from a data-pack markdown file.
...
def format_report(r: GGResult) -> str:
    ...
    # Title
    L.append(f"# {r.stock_code} -- Quality Yield GG Penetration Return Analysis")
    L.append("")
    ...
    L.append(_row("Standard QY GG", r.gg_standard))
    ...
    L.append("---")
    L.append("")
    L.append("*Generated by US Equity Quality Yield GG Calculator*")
...
def _print_summary(r: GGResult) -> None:
    ...
    w = 70
    print(f"\n{'=' * w}")
    print("Quality Yield GG Calculator -- US Equity")
    print(f"{'=' * w}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="US Equity Quality Yield GG Calculator",
